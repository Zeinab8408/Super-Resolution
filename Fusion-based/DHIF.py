### DHIF-Net : Deep Hyperspectral Image Fusion Network With Iterative Spatio-Spectral Regularization ###
### doi: 10.1109/TCI.2022.3152700 ###
### Implementation Link : https://github.com/TaoHuang95/DHIF-Net ###
...
class Decoder(nn.Module):
    def __init__(self, Ch = ms_channels, kernel_size=[7, 7, 7]):
...
class HSI_Fusion(nn.Module):
    def __init__(self, Ch, stages, sf):
        ...
        self.RT = nn.Sequential(nn.Conv2d(pan_channels, Ch, kernel_size=3, stride=1, padding=1), nn.LeakyReLU())
        self.R  = nn.Sequential(nn.Conv2d(Ch, pan_channels, kernel_size=3, stride=1, padding=1), nn.LeakyReLU())
        ...
        self.conv  = nn.Conv2d(Ch+pan_channels, 64, kernel_size=3, stride=1, padding=1)
        ## Dense connection
...
W = PAN.shape[2]
w = LRHS.shape[2]
net = HSI_Fusion(Ch=hs_channels, stages=4, sf=int(W/w)).to(device)
optimizer = optim.Adam(net.parameters(), lr=0.0003, betas=(0.9, 0.999), eps=1e-8)
